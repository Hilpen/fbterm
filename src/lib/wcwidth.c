...
static const unicode_range_t ZEROES[] = {
...
static const unicode_range_t WIDES[] = {
...
bool static search_in_table(wchar_t usc, const unicode_range_t tab[],
        int lo, int hi) {
    // binary search
    while (lo < hi) {
        int mid = (lo + hi) >> 1;
        usc < tab[mid].start ? (hi = mid) :
                               (lo = mid + 1);
    }
    // finally lo will be the first element greater than usc
    // return false if [lo - 1, lo) is out of range or usc is not in tab[lo - 1]
    return !(--lo < 0 || tab[lo].end < usc);
};

int wcwidth(wchar_t usc) {
    static int ZEROES_LEN = sizeof(ZEROES) / sizeof(unicode_range_t);
    static int WIDES_LEN = sizeof(WIDES) / sizeof(unicode_range_t);

    // C0/C1 control charaters
    if (usc < 0x20 || (0x7F <= usc && usc <= 0x9F)) return -1;
    // shortcut for mostly used Latin1 characters
    if (usc < 0x300) return 1;

    if (search_in_table(usc, ZEROES, 0, ZEROES_LEN)) return 0;

    return 1 + (search_in_table(usc, WIDES, 0, WIDES_LEN) ? 1 : 0);
}
